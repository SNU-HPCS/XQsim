...
def gen_conn_bd(data_bw):
    ...
    for w in range(0, data_bw):
        ...
        entry["B_type"] = "input_wren"
        entry["B_name"] = "wren"
        entry["B_depth_split"] = ceil(log(data_bw, 2))+1
        ...
        entry["Depth"] = 1
        entry["A_type"] = "input_wren"
        entry["A_name"] = "wrin"
        entry["A_depth_split"] = ceil(log(data_bw, 2))+1
        entry["A_dist_loop"] = None
        entry["B_type"] = None
        entry["B_name"] = None
        ...
        ## dout
        entry = dict()
        entry["Type"] = "output_dout[{}]".format(w)
        entry["Name"] = "dout[{}]".format(w)
        entry["Depth"] = 3
...
def gen_ndrobuf_netlist(data_bw, rsfqlib):
    initial_setup(rsfqlib)
    return gen_conn_bd(data_bw)
